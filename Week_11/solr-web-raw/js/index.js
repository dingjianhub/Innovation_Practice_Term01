...
function searchPaper(key) {
    ...
    $("#result").empty();
    $.getJSON({
        url: "http://localhost:8983/solr/search_test/select?q=" + key + "&wt=json&indent=true&json.wrf=?",
...
function getSuggest() {
    ...
    // 向服务器请求联想词
    $.getJSON({
        url: "http://localhost:8983/solr/sug_paper_keywords/suggest?q=" + $("#keyword").val() + "&wt=json&indent=true&json.wrf=?",
        success: function(result) {
            // 获取返回的数据中我们需要的部分
            res = result.suggest.mySuggester[$("#keyword").val()];
            //res.suggestions = res.suggestions.slice(0,4);
            //控制
            //alert(1)
            if (res.suggestions.length) {
                // 利用for插入每一个结果
                //console.log("本次数据长度" + res.suggestions.length);
